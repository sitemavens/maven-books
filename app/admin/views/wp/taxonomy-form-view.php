...
<div >	
	<table class="form-table" ng-controller="TaxonomyController">
		<tr class="is-smart-rules-form">
			...
			<td>
				<input type="checkbox" name="mvn_smart_rules[is_smart_term]" ng-model="isSmartTerm"
				...
				<div class="mvn-shop-category-form-container clear clearfix">
					...
					<ul class="smart-rules">
						<li class="smart-rule" ng-repeat="termRule in smartTermRules">
							...
							<select class="add-rule" name="mvn_smart_rules[mvn_shop_term][smart_rules][{{$index}}][field]" ng-model="termRule.field" ng-options="i as value for (i,value) in smartTermFields">
							</select>
							<select class="add-rule" name="mvn_smart_rules[mvn_shop_term][smart_rules][{{$index}}][operator]" ng-model="termRule.operator" ng-options="i as value for (i,value) in smartTermOperator">
							</select>
							<input type="text" ng-if="termRule.field && termRule.operator" name="mvn_smart_rules[mvn_shop_term][smart_rules][{{$index}}][rule]" class="col-sm-5" ng-model="termRule.rule"/>
						</li>
